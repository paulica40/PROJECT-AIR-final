...
import * as dataFormat from 'pages/CRUD/Indicatori/table/IndicatoriDataFormatters';

import actions from 'store/actions/indicatori/indicatoriListActions';
...
const IndicatoriTable = () => {
  const [filters, setFilters] = React.useState<any[]>([{ label: 'Nume', title: 'Nume' }]);
  ...
  const loading = useTypedSelector((store) => store.indicatori.list.loading);
  const rows = useTypedSelector((store) => store.indicatori.list.rows);
  const count = useTypedSelector((store) => store.indicatori.list.count);
  const modalOpen = useTypedSelector((store) => store.indicatori.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.indicatori.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color="default"
            size="xs"
            onClick={() => dispatch(push(`/admin/indicatori/${cell}`))}
          >
            View
          </Button>
        )}
        <Button
          color="info"
          size="xs"
          onClick={() => history.push(`/admin/indicatori/${cell}/edit`)}
        >
          Edit
        </Button>
        &nbsp;&nbsp;
  ...
  return (
    <div>
      <Widget title={<h4>Indicatori</h4>} collapse close>
        <Link to="/admin/indicatori/new">
