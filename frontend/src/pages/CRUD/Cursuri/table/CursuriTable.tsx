...
import * as dataFormat from 'pages/CRUD/Cursuri/table/CursuriDataFormatters';

import actions from 'store/actions/cursuri/cursuriListActions';
...
const CursuriTable = () => {
  ...
  const loading = useTypedSelector((store) => store.cursuri.list.loading);
  const rows = useTypedSelector((store) => store.cursuri.list.rows);
  const count = useTypedSelector((store) => store.cursuri.list.count);
  const modalOpen = useTypedSelector((store) => store.cursuri.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.cursuri.list.idToDelete);
      ...
      <div>
        {null && (
          <Button
            color="default"
            size="xs"
            onClick={() => dispatch(push(`/admin/cursuri/${cell}`))}
          >
            View
          </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/cursuri/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
        <Button
  ...
  return (
    <div>
      <Widget title={<h4>Cursuri</h4>} collapse close>
        <Link to="/admin/cursuri/new">
